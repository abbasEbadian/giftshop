...
function Card({data, addToCard=false, showRate=false, favoriteAndRate=false, layered=false}) {
  ...
  return <div className="single-card">
    <div className="data-container position-relative">
        ...
        <Link href={{pathname: "/product/[slug]", query: {slug: +data.id}}}><a>
        <div className="data position-absolute p-3 top-0 text-white w-100 h-100 d-flex flex-column  justify-content-between">
            <div className="d-flex align-items-center justify-content-between">
                <span dir="ltr">${Number(data.realPrice).toLocaleString()}</span>
                <span>{capit(data.category)} {" "} Card</span>
            </div>
            <div className="d-flex align-items-center justify-content-between">
                {showRate?<span>{data.rate} {" "} <StarIcon/></span>:<i></i>}
    ...
    {addToCard?<div className="add-to-card-container d-flex justify-content-between align-items-center">
    ...
    {favoriteAndRate?<>
        <div className="d-flex align-items-center justify-content-evenly mt-3">
            <span className="cursor-pointer">افزودن به کارت های مورد علاقه</span>
            <BookmarkBorderIcon />
        </div>
        <div className="d-flex align-items-center justify-content-evenly mt-3" dir="ltr"> 
            <Rating
